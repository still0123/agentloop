...
- **上下文工程**：管理对话历史与工具结果，通过分级压缩控制上下文大小，保留任务目标、约束和近期执行信息。
...
实现“大结果落盘 → 旧历史归档 → 旧结果占位 → 模型摘要”的四级压缩管线，
按需逐级压缩，减少重复传入的内容。压缩过程中保护工具调用与工具结果的配对关系；
模型报告上下文超限时，再触发压缩并重试。
...
上下文压缩保留本次用户请求；旧工具结果先保存原文再替换为路径，摘要服务失败时
降级为请求、近期消息和归档路径。方案比较、字段设计和离线实验见
...
完整原理、消息切片图和配对不变量见
**[AgentLoop 实现详解：上下文压缩](docs/study-guide.md#9-上下文压缩)**。
