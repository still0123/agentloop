...
**用于本地开发任务的可扩展 Coding Agent**

`Python 3.10+` · `OpenAI-compatible / Anthropic` · `CLI / Web / macOS` · `MIT`

[核心能力](#核心能力) · [架构](#架构) · [上下文机制](#上下文机制) · [验证](#验证) · [快速开始](#快速开始)

</div>

---

AgentLoop 把模型推理、工具调用、权限审批和上下文管理组织成一个本地 Agent Loop。
它面向代码、文件和命令行任务：模型决定下一步行动，工具执行并返回观察结果，循环持续到模型给出最终回答或达到运行保护条件。
