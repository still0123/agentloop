...
AgentLoop 被用于流水线排障机器人：通过飞书接收流水线输入，固定本次 Run / Job / Attempt，采集失败用例，沿已核验的请求映射查询服务端日志，整理带原文定位和未知项的诊断结果。应用目标是取得足以解释直接失败的关键证据，并明确尚未确定的深层原因。

这一场景需要持续处理长日志、跨服务请求标识和多轮调查状态，推动了核心运行时的修复与增强：

| 场景中的问题 | 核心机制 | 代码入口 |
|---|---|---|
| 长日志超出上下文预算，压缩后关键证据难以找回 | 原始结果归档、稳定引用、分页回读与完整性校验 | [compact.py](agentloop/compact.py) / [artifacts.py](agentloop/artifacts.py) |
| 摘要失败后新增调查历史难以恢复 | 保留有效摘要及待处理历史，后续压缩重放 | [compact.py](agentloop/compact.py) |
| 模型依据旧摘要误判当前调查状态 | 每轮通过 `context_provider` 注入程序维护的最新状态，并纳入请求预算 | [agent.py](agentloop/agent.py) / [budget.py](agentloop/budget.py) |
| 模型请求本轮已撤下的工具 | `RoundControl` 在执行层校验本轮可用工具范围 | [control.py](agentloop/control.py) / [agent.py](agentloop/agent.py) |
| 调查占满预算，缺少报告提交机会 | 预留收尾轮次、限制收尾工具，支持提交成功后退出 | [agent.py](agentloop/agent.py) |
| 长请求标识易混淆，来源关系难保留 | `ScopedReferences` 保存稳定短引用、作用域、类型、来源和父引用 | [references.py](agentloop/references.py) |
